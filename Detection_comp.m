...
%%
jj=4; % scale for desired magnitude 
for h=1:179
...
%                 
       SNRE(h,:)=abs(diff(record_E(h,:),2))/rms(nt); %signal to noise ratio;
        SNRN(h,:)=abs(diff(record_N(h,:),2))/rms(nt);
        SNRZ(h,:)=abs(diff(record_Z(h,:),2))/rms(nt);
...
for ff=1:179
    ...
    recordZsurf=rsac([record_name 'Z.SAC']);
     ...
     record_name=strcat('/Users/marusiak/Documents/Gattaca/Europa_constant_Tc/Europa_',num2str(ice_thickness),...
        'km_porous_deep/',num2str(ff,'%03d'),'_MX');
...
plot(1:179,smooth(record_E_ratio),'b-',1:179,smooth(record_N_ratio),'r-',...
    1:179,smooth(record_Z_ratio),'k-','LineWidth',2)
ylabel('RMS Ratio Porous/Solid')
xlabel('Distance (deg)')
title([num2str(ice_thickness) 'km thick Deep'])
...
for ff=1:179
    ...
    recordZsurf=rsac([record_name 'Z.SAC']);
     ...
     record_name=strcat('/Users/marusiak/Documents/Gattaca/Europa_constant_Tc/Europa_',num2str(ice_thickness),...
        'km_porous_surface/',num2str(ff,'%03d'),'_MX');
...
ylabel('RMS Ratio Porous/Solid')
xlabel('Distance (deg)')
title([num2str(ice_thickness) 'km thick Shallow'])
...
for ff=1:179
    ...
    recordEsurf=rsac([record_name 'E.SAC']);
    recordNsurf=rsac([record_name 'N.SAC']);
    recordZsurf=rsac([record_name 'Z.SAC']);%Europa_detection_comparison
% Created by AGM on 27 April 2021
% Uses noise models from Panning. PlanetProfile creates structure models,
% Axisem/Instaseis generates waveforms. 
clear
...
%%
jj=3; % scale for desired magnitude 
for h=1:179
    %times=tauptime('mod',model,'deg',h);
    %for k=1:length(times);
        seismoE(h,:)=diff(record_E(h,:),2)'.*scale(jj)+nt;
